...
| 버전 | 날짜 | 변경 내용 | 상태 |
|---|---|---|---|
| v1.0 | 2026-05-15 | 초기 버전: 분석+번역 통합 프롬프트 | 구버전 |
| v1.1 | 2026-05-15 | methodology_ko / results_ko / limitations_ko 필드 추가 | 구버전 |
| v1.2 | 2026-05-16 | 번역 품질 개선: 금지 패턴 명시 + 자연스러운 번역 원칙 + temperature 0.5 + max_tokens 2500 | ✅ 현재 사용 |

---

## v1.2 — 번역 품질 개선 (금지 패턴 + 자연스러운 번역 원칙)
...
| temperature | 0.5 (v1.1: 0.3) |
| max_tokens | 2500 (v1.1: 1500) |
| 예상 입력 토큰 | ~950 tokens (+150, 금지 패턴 예시 추가분) |
| 예상 출력 토큰 | ~900 tokens (토큰 여유로 번역 품질 향상 기대) |
| 논문당 예상 비용 | ~$0.017 (GPT-4o 기준, +$0.005) |

### 변경 이유

v1.1까지 번역 결과가 영어 어순을 그대로 따르는 직역체가 많아 가독성 저하.
주요 원인: temperature 0.3이 너무 낮아 창의적 표현 선택 억제, max_tokens 1500이 빠듯해 뒷부분 압축, 번역 규칙이 추상적("자연스러운 한국어")이어서 모델이 방향을 잡지 못함.

### 핵심 변경 사항

1. **금지 패턴 명시**: ❌/✅ 예시로 나쁜 번역 vs 좋은 번역을 구체적으로 제시
2. **자연스러운 번역 원칙**: 60자 초과 시 문장 분리, 능동태 선호, 자연스러운 접속어 목록
3. **temperature 0.3 → 0.5**: 직역 경향 줄이고 자연스러운 표현 선택 유도
4. **max_tokens 1500 → 2500**: 토큰 여유 확보로 번역 압축 방지

### 롤백 조건

번역 품질이 오히려 저하되거나 JSON 파싱 실패율이 증가하면 즉시 v1.1로 롤백:
```yaml
# config.yaml
prompt_version: "v1.1"
llm:
...
## v1.1 — methodology / results / limitations 추가
...
| 버전 | 1.1 |
| 작성일 | 2026-05-15 |
| 아카이브 | `agent/prompts/archive/v1.1_unified.txt` |

### 변경 이유

v1.0은 기본 번역만 제공. 독자가 논문의 방법론과 실험 결과를 파악하려면 원문을 직접 읽어야 하는 불편함.
methodology_ko / results_ko / limitations_ko 세 필드를 추가해 논문 한 페이지 요약 완성.
